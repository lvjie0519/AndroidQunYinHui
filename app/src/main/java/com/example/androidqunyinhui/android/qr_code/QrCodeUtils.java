...
public class QrCodeUtils {
    ...
    public static Bitmap syncEncodeQRCode(String content, int size){
        return syncEncodeQRCode(content, size, Color.parseColor("#000000")  , Color.parseColor("#ffffff") , null);
    }

    /**
     * 同步创建指定前景色、指定背景色、该方法是耗时操作，请在子线程中调用。
     *
    ...
    public static Bitmap syncEncodeQRCode(String content, int size, int foregroundColor, int backgroundColor, Bitmap logo) {
        ...
        try {
            ...
            return addLogoToQRCode(bitmap, null);
        } catch (Exception e) {
            return null;
        }
    }
